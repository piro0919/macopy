...
/// Password managers and similar apps declare marker types alongside the value
/// they put on the pasteboard. `org.nspasteboard.ConcealedType` means the
/// content is a secret, `TransientType` that it is not meant to be kept, and
/// `AutoGeneratedType` that the app produced it rather than the user copying
/// something. Clipboard managers are expected to leave all three alone.
/// See http://nspasteboard.org/
#[cfg(target_os = "macos")]
const PRIVATE_MARKERS: [&[u8]; 3] = [
    b"org.nspasteboard.ConcealedType\0",
    b"org.nspasteboard.TransientType\0",
    b"org.nspasteboard.AutoGeneratedType\0",
];

/// Takes the pasteboard so the test can pass a private one instead of the
/// general pasteboard, which would clobber whatever the user has copied.
#[cfg(target_os = "macos")]
unsafe fn pasteboard_is_marked_private(pasteboard: id) -> bool {
    ...
    for marker in PRIVATE_MARKERS {
        let name: id = msg_send![objc::class!(NSString), stringWithUTF8String: marker.as_ptr()];
...
#[cfg(target_os = "macos")]
fn clipboard_is_marked_private() -> bool {
    unsafe {
        let pool = NSAutoreleasePool::new(nil);
        let pasteboard: id = msg_send![objc::class!(NSPasteboard), generalPasteboard];
        let marked = pasteboard_is_marked_private(pasteboard);
        let _: () = msg_send![pool, drain];
        marked
    }
}
...
fn update_clipboard(app: &AppHandle, state: &AppState) -> bool {
    // Never take a copy the source app marked as a secret, as transient, or as
    // something it generated itself. The watcher has already advanced its
    // change count, so skipping here does not make it retry the same item.
    if clipboard_is_marked_private() {
        return false;
    }
...
mod tests {
    ...
    /// Uses a uniquely named pasteboard rather than the general one, so running
    /// the tests does not touch whatever the user has on their clipboard.
    unsafe fn scratch_pasteboard() -> id {
        msg_send![objc::class!(NSPasteboard), pasteboardWithUniqueName]
    }

    unsafe fn ns_string(bytes: &[u8]) -> id {
        msg_send![objc::class!(NSString), stringWithUTF8String: bytes.as_ptr()]
    }

    #[test]
    fn plain_text_is_not_marked_private() {
        unsafe {
            let pool = NSAutoreleasePool::new(nil);
            let pb = scratch_pasteboard();
            let types: id = msg_send![objc::class!(NSArray),
                arrayWithObject: ns_string(b"public.utf8-plain-text\0")];
            let _: i64 = msg_send![pb, declareTypes: types owner: nil];
            let _: bool = msg_send![pb, setString: ns_string(b"hello\0")
                forType: ns_string(b"public.utf8-plain-text\0")];

            assert!(!pasteboard_is_marked_private(pb));
    ...
    fn each_marker_is_recognised() {
        for marker in PRIVATE_MARKERS {
            unsafe {
                let pool = NSAutoreleasePool::new(nil);
                let pb = scratch_pasteboard();
                let types: id = msg_send![objc::class!(NSMutableArray), array];
                let _: () = msg_send![types, addObject: ns_string(b"public.utf8-plain-text\0")];
                let _: () = msg_send![types, addObject: ns_string(marker)];
                let _: i64 = msg_send![pb, declareTypes: types owner: nil];
                let _: bool = msg_send![pb, setString: ns_string(b"secret\0")
                    forType: ns_string(b"public.utf8-plain-text\0")];

                assert!(
                    pasteboard_is_marked_private(pb),
                    "expected {} to be skipped",
                    String::from_utf8_lossy(&marker[..marker.len() - 1])
                );
